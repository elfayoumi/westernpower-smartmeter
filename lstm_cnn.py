...
class TsCNNLstm(BaseEstimator, ClassifierMixin):
    DAYS_AHEAD = 1
    LAGGED_DAYS = 0
    VALID_DAYS = 15
    ...
    def __init__(self, verbose=1, batch_size=128, epochs=200, lr=0.001, sequence_length=20, l2_reg=0.01, logger=None,
        ...
        self.D = ts_dim
        self.lr = lr
        self.name = 'lstm_cnn_multiInput'
        self.data_dir = os.path.join(self.current_directory, 'data')
        self.batch_size = batch_size
        self.epochs = epochs
        self.verbose = verbose
        self.ts_scaler = StandardScaler(copy=True)
        self.y_scaler  = MinMaxScaler(feature_range=(0,0.9), copy=True)
        self.window_size = sequence_length
        self.l2_reg = l2_reg
        self.pickle_file = os.path.join(self.output_directory, 'ts_1.pickle')
        self.n_steps = n_steps

    def find_categorical(self, X):
        # Get list of categorical column names
        self.categorical_columns = ['LCLid', 'Acorn', 'Acorn_grouped', "icon", "stdorToU", "Type", "day.of.week",
                                    'precipType', 'summary', 'before_holiday', 'after_holiday', 'month', 'year']
    ...
    def proces_categorical_columns(self, df):
        ...
        self.ts_scaler.fit(df[self.non_categorical_columns].fillna(method='ffill').values.reshape(-1, len(self.non_categorical_columns)))
        self.y_scaler.fit(df[self.label_column].fillna(method='ffill').values.reshape(-1, 1))

    def create_dataset(self, df):
        self.D = len(self.non_categorical_columns)
        ...
        self.info(df.dtypes)
        self.df = df
        self.y = self.y_scaler.fit_transform(y.reshape(-1, 1))
        self.ts, idz = self.window_transform_series(self.window_size, df.index)

        return (self.df, self.ts, idz, self.y)

    def window_transform_series(self, window_size, index):

        v = self.df[self.non_categorical_columns]
        D = len(self.non_categorical_columns)
        ...
        for lclid, new_df in v.groupby(level=0):
            ...
            for id in d:
                r = new_df.loc[lclid][:id].values[-window_size:]
                if (r.shape[0] >= window_size):
                    # s = np.zeros((window_size - r.shape[0], r.shape[1]), dtype=float)
                    # r = np.insert(r, 0, s, axis=0)
                    X.append(r.reshape(1, D*window_size))
                    idx.append([lclid, id])
                    i = i +1
                if i > 1e5:
                    break
        ...
        # reshape each
        self.log.info('done data')

        X = np.asarray(X)
        self.log.info(f'converted to numpy: {X.shape}')
    ...
    def separate_train_valid(self, X, ts, idz, y):
        max_date = max(idz[:,1])

        valid_start_day = max_date - timedelta(days=TsCNNLstm.VALID_DAYS)
        train_idx = list(filter(lambda t: t[1] < valid_start_day, idz))
        valid_idx = list(filter(lambda t: t[1] >= valid_start_day, idz))
        ts = pd.DataFrame(ts.reshape(-1, self.window_size * self.D), index=idz)
        ...
        train_idx = list(map(lambda t: (t[0], t[1].date()),  train_idx))
        valid_idx = list(map(lambda t: (t[0], t[1].date()), valid_idx))
    ...
    def model_setup(self):
        ...
        seq_model = Sequential(
            (TimeDistributed(Conv1D(filters=32, kernel_size=3, activation='tanh'), input_shape=input_shape),
             TimeDistributed(AveragePooling1D(pool_size=4)),
             TimeDistributed(Conv1D(filters=16, kernel_size=3, activation='tanh')),
             TimeDistributed(AveragePooling1D(pool_size=4)),
             TimeDistributed(Flatten())))
        seq_input = seq_model.input
        #seq_input.name = 'time_distributed_input'
        self.seq_name = seq_input.name
        self.seq_name = self.seq_name[:self.seq_name.index(":")]
    ...
    def fit_data(self, show_figures=True):
        ...
        train_values[self.seq_name] = np.asarray(np.reshape(self.ts_train, (self.ts_train.shape[0], self.n_steps, self.window_size//self.n_steps, self.D)))
        ...
        val_values[self.seq_name] = np.asarray(np.reshape(self.ts_valid, (self.ts_valid.shape[0], self.n_steps, self.window_size//self.n_steps, self.D)))
    ...
    def predict(self, X, ts, y=None):
        try:
            ...
            test_values[self.seq_name] = ts.reshape (-1, self.n_steps, self.window_size // self.n_steps, len(self.non_categorical_columns))
...
if __name__ == "__main__":
    ...
    try:
        ...
        lclids = list(set(np.unique(np.asarray(list(map(lambda t: t[0], X_train.index))))) & set(np.unique(np.asarray(list(map(lambda t: t[0], X_valid.index))))))
        ...
        predict_y_train = keras_multinput.unscale_y_value(keras_multinput.predict(X_train, ts_train.values.reshape(-1,
                                                                                                                   keras_multinput.window_size,
                                                                                                                   keras_multinput.D)).reshape(
            -1, 1))
        predict_y_valid = keras_multinput.unscale_y_value(keras_multinput.predict(X_valid, ts_valid.values.reshape(-1,
                                                                                                                   keras_multinput.window_size,
                                                                                                                   keras_multinput.D)).reshape(
            -1, 1))
        ...
        predict_y_train = predict_y_train.reset_index().merge(acorn_Lclid, how='inner', left_on=['index'],
                                                              right_on=['LCLid'])
        predict_y_valid = predict_y_valid.reset_index().merge(acorn_Lclid, how='inner', left_on=['index'],
                                                              right_on=['LCLid'])
        logger.info(predict_y_train.head())
